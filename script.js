...
// EXPORTAR PARA HTML INDEPENDENTE
function downloadMapa() {
    if (mapLayers.getLayers().length === 0) return alert("Gere o mapa antes de baixar!");

    // 1. Scanner inteligente: Extrai pontos soltos e entra dentro de Grupos (Linhas/Polígonos)
    function extrairFeicoes(grupo, popupHerdado = null, corHerdada = null) {
        let feicoes = [];
        grupo.eachLayer(function(layer) {

            // Verifica se este nível tem um popup ou tooltip próprio
            let popupAtual = popupHerdado;
            if (layer.getPopup && layer.getPopup()) {
                popupAtual = layer.getPopup().getContent();
            } else if (layer.getTooltip && layer.getTooltip()) {
                popupAtual = layer.getTooltip().getContent();
            }

            // Verifica se tem uma cor definida neste nível
            let corAtual = layer.options ? (layer.options.color || corHerdada) : corHerdada;

            // Se for um Grupo de Camadas (Linhas ou Polígonos complexos), a função chama a si mesma para entrar nele
            if (layer instanceof L.LayerGroup) {
                feicoes = feicoes.concat(extrairFeicoes(layer, popupAtual, corAtual));
            } 
            // Se for a forma geométrica final, extrai para GeoJSON
            else if (layer.toGeoJSON) {
                let geojson = layer.toGeoJSON();
                if (!geojson.properties) geojson.properties = {};
                
                // Acopla o popup (seja dele mesmo ou herdado da pasta pai)
                if (popupAtual) {
                    geojson.properties.popup = popupAtual;
                }
                
                // Salva as configurações visuais (cores, grossura da linha, etc)
                if (layer.options) {
                    geojson.properties.color = layer.options.color || corAtual || '#3388ff';
                    geojson.properties.fillColor = layer.options.fillColor || layer.options.color || corAtual;
                    geojson.properties.weight = layer.options.weight || 3;
                    if (layer.options.radius) geojson.properties.radius = layer.options.radius;
                }
    ...
    // Executa o scanner no mapa atual
    const featuresExtraidas = extrairFeicoes(mapLayers);
    const geojsonData = { type: "FeatureCollection", features: featuresExtraidas };

    // 2. Proteção contra a quebra do navegador (dividindo as tags de script)
    ...
    // 3. Monta a página HTML final de exportação
    const htmlContent = `<!DOCTYPE html>
        ...
        <body>
            ...
            ${scriptStart}
                ...
                var layer = L.geoJSON(data, {
                    ...
                    pointToLayer: function(feature, latlng) {
                        if (feature.properties.radius) {
                            return L.circle(latlng, {radius: feature.properties.radius, color: feature.properties.color, fill: true});
                        }
                ...
                // Foca a câmera nos dados desenhados
    ...
    // 4. Executa o Download do Arquivo
    ...
    for (let i = 0; i < excelData.length; i++) {
        ...
        try {
            ...
            // MODO ENDEREÇO
                ...
                if (tipoGeo === "Point") {
                    const urlArcgis = 
                    `https://geocode.arcgis.com/arcgis/rest/services/World/GeocodeServer/findAddressCandidates?f=json&singleLine=${encodeURIComponent(busca + ", Brasil")}&maxLocations=1`;
                    ...
                    let cidade = busca.split(',')[-1] ? busca.split(',')[1].trim() : "Balneário Camboriú";
                    if (!nomeRua) continue;
